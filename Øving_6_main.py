...
with open("DAT 120 - programmering/Øvinger/Øving_6_prosjektoppgave/LOKAL.csv", "r") as file:
        for row in file:
            Del = row.strip().split(';')
            if len(Del) >= 5:
                time = Del[0]
...
with open("DAT 120 - programmering/Øvinger/Øving_6_prosjektoppgave/SOLA.csv", "r") as file:
        for row in file:
            Del = row.strip().split(';')              #Strip fjerner mellomrom etc., split lager elementer ved ;
            if len(Del) >= 5:                      
